...
# Day 3 Intro
From what we've covered so far, you should be in a good position to wrangle a lot of different types of data. The tools from Days 1 and 2 will get you very far. Today we'll look at some interesting things for specific types of data, then wrap back around to some more common R techniques, and wrap up with preparation for tomorrow's hackathon. We'll be making teams and (starting to) select data sets for tomorrow at the end of today's class, so start thinking about potential teammates (2-4 students per team), team names, and data set ideas.
...
## Your turn
Spend some time doing fuzzy joins on these two data sets. Vary the join type, distance metric, and threshold. 
