...
## SPSS and Haven
The user_na option in haven::read_spss affects how the NA values are stored internally. If you have missing data that is missing for different reasons, sometimes it is important to take that into account. For now, we can ignore it and read the data in without that option (user_na defaults to FALSE):
...
## Question about Argument Placement
Remember this?
...
Why is it the case that certain arguments (e.g. color) are inside aes() while others (e.g. alpha) are not?

## Day 1 Exercises Revisited
Let's look at one way of working through them.
...
The following chunk of code needs the <span class="func">as.data.frame()</span>. Why, you might ask? Almost everything in <span class="pack">dplyr</span> converts data to a <span class="pack">tibble</span>. What's a <span class="pack">tibble</span>? [From the tidyverse website](https://tibble.tidyverse.org/):
...
Many base R functions will go crazy when they encounter a tibble, so you need to explicitly make it a data frame. You might ask what is the trouble with tibbles (anyone?)...
...
Spend some time pivoting different subsets of the starwards data set. 
...
We've made it to the end of Day 2, which means we're more than halfway through the data wrangling material that we'll cover (but certainly not halfway through all of the data wrangling tools out there). For today's assessment I'd like to you think about a problem or data set that you've encountered. How can the tools we've learned so far help?

Specifically, think about (and the discuss):

- A data set that you've worked with, a data set you would like to work with, or a business problem that you have come across where a large (and possibly messy) data set would be useful for solving the problem.
- What steps would you need to do to get something out of the data? *Something* here is intentionally vague, and could refer to summary statistics, visualizations, subsets of the data, or something else. Steps here should be tool-agnostic.
- What tools would you use to accomplish the goals in the previous step? If you have come up with steps that involve tools we haven't covered, I encourage you to browse through the rest of the [tidyverse](https://www.tidyverse.org/packages/). 
